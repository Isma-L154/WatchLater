<script lang="ts">
	 ...
	 *
	 * A plain link (not a form) because the OAuth handshake is a GET redirect —
	 * which also means it keeps working with JavaScript disabled.
	 */
	interface Props {
		/** `full` stretches to the container; `compact` is the header pill. */
		size?: 'full' | 'compact';
		label?: string;
	}

	let { size = 'compact', label = 'Sign in with Google' }: Props = $props();
</script>
